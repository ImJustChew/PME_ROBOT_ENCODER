...
// read motor state from serial
    // if(Serial.available() > 0) {
    //     motorState = (MotorState)Serial.parseInt();
    //     setMotorState(motorState);
    // }

  // switch setpoint1 0 and 0.4 every 5 seconds
    ...
    setMotorState(STOP);
    delay(5000);
    setMotorState(MOVE_LEFT);
    delay(5000);
    setMotorState(STOP);
    delay(5000);
    setMotorState(MOVE_RIGHT);
    delay(5000);
    setMotorState(STOP);
    delay(5000);
}
